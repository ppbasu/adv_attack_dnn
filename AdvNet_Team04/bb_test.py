...
parser.add_argument('--pgd', type=int, default=0, help='if True, pgd attack generated inputs are used for evaluation')
...
parser.add_argument('--base', type=str, metavar='FILE',
                    help='base model FILE on validation set')
parser.add_argument('--basemodel', metavar='MODEL', default='alexnet',
                    choices=model_names,
                    help='model architecture: ' +
                    ' | '.join(model_names) +
                    ' (default: alexnet)')
parser.add_argument('--advonly', type=int, default=0, help='if True, pgd attack generated inputs are saved')
...
def forward(data_loader, model, criterion, epoch=0, training=True, optimizer=None, pgd=False, savepgd=False):
    ...
    # attack = PGD(model_base, eps=0.031, alpha=2/255, iters=10)
    ...
    for i, (inputs, target) in enumerate(data_loader):
        ...
        # if pgd:
            # inputs=pgd_attack(inputs, target)
        
        if args.gpus is not None:
            target = target.cuda()
            # target_adversarial = target_adversarial.cuda()
        ...
        # target_adversarial_var = Variable(target_adversarial)
        # compute output
        ...
        # loss_adversarial = criterion(output_adversarial, target_adversarial_var)
        ...
        # if type(output_adversarial) is list:
            # output_adversarial = output_adversarial[0]
...
def train(data_loader, model, criterion, epoch, optimizer, pgd=False, savepgd=False):
    # switch to train mode
    model.train()
    return forward(data_loader, model, criterion, epoch,
                   training=True, optimizer=optimizer, pgd=pgd, savepgd=savepgd)


def validate(data_loader, model, criterion, epoch, pgd=False, savepgd=False):
    # switch to evaluate mode
    model.eval()
    return forward(data_loader, model, criterion, epoch,
                   training=False, optimizer=None, pgd=pgd, savepgd=savepgd)
