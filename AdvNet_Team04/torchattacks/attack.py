...
class Attack(object):
    ...
    def save(self, file_name, data_loader, accuracy=True, orig = True, advonly = False):
        
        self.model.eval()
        
        image_list = [] #adv images
        original_images = [] #clean images
        label_list = [] #clean image labels
        ...
        predicted_original = [] #clean image predictions
        predicted_attack = [] #adv image predictions
        ...
        # for step, (images, labels) in enumerate(data_loader) :
            ...
            # predicted_original.append(pred.cpu())
            
            # outputs = self.model(adv_images.float())
            # _, pred = torch.max(outputs.data, 1)
            
            # predicted_attack.append(pred.cpu())
        ...
        for step, (images, labels) in enumerate(data_loader) :
            ...
            adv_images = adv_images.float() / adv_images.max()
        ...
        # z = torch.cat(label_list, 0)
        # i = torch.cat(predicted_original, 0)
        # j = torch.cat(predicted_attack, 0)
        # torch.save((x, y, z, i, j), file_name)
